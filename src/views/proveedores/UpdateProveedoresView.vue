<template>
    ...
    <h1>{{ isEditing ? 'Editar Proveedor' : 'Creación de Proveedor' }}</h1>
...
export default {
  ...
  methods: {
    async loadProveedor() {
      try {
        const response = await axios.get(`http://localhost:4200/proveedores/${this.idProveedores}`);
